...
con= dbConnect(DBI::dbDriver("Oracle"), oracle.username, oracle.password, oracle.server)
  ...
  logbook= dbGetQuery(con, " SELECT * FROM MARFISSCI.MARFIS_CRAB MARFIS_CRAB
WHERE (MARFIS_CRAB.TARGET_SPC=705)")



      lic = dbGetQuery(con, "select * from MARFISSCI.LICENCE_AREAS")
      # areas = dbGetQuery(con, "select * from marfis.areas")
...
q=paste("C:/Rsaves/fishery",iy,sep="/")
dir.create(q)
wd=q
setwd(q)
print(paste("All figures will be stored at: ",q))


  # tmp SAVES
  save(logbook, file=paste("logbook.",iy,".rdata", sep=""), compress=T)
  save(lic, file=paste("lic.",iy,".rdata", sep=""), compress=T)



  # load saved temporary files
  load(paste("logbook.",iy,".rdata", sep=""))
  load(paste("lic.",iy,".rdata", sep=""))
...
#
  ...
  # licence information
    ...
    #	descr = merge(areas, lic0, by="area_id", all.x=F, all.y=F, sort=F)
...
logs$date_landed[i4x]=logs$date_landed[i4x]-7776000 #subtracts 91 days from all dates ( 7776000seconds)
logs$years=as.character(years(as.chron(logs$date_landed)))        #determine year
logs$date_landed[i4x]=logs$date_landed[i4x]+7776000 #adds 90 days

      logs$year_q=paste(logs$year, logs$q, sep=":")


save(logs, file=paste("logs.",iy,".rdata", sep=""), compress=T)
load(paste("logs.",iy,".rdata", sep=""))
...
write.table(tables,file="tables.csv", sep=",")
...
year.land=as.data.frame(xtabs(land$pro_rated_slip_wt_lbs~land$years+land$cfa0), stringsAsFactors=F)
...
write.table(tot.lic,file="Landings by Licence.csv", sep=",")
...
crablandings = crablandings[ is.finite(crablandings$pro_rated_slip_wt_lbs) ,]
 ...
 crablandings$year=median(landings$year)

  write.table(crablandings,file="Landings by Week", sep=",")

#landings by licence
#liceland = compute.sums( x=landings, var="pro_rated_slip_wt_lbs", index=c("licence_id", "cfa0")  )
#liceland = liceland[is.finite(liceland$pro_rated_slip_wt_lbs),]
#liceland$mt=NA
#liceland$mt=(liceland$pro_rated_slip_wt_lbs)/2204.626
#
...
#create metafiles:
 ...
 for (c in cfa)   {
 filename=paste(iy,"_", c,"_weekly_landing", ".emf", sep="")
    win.metafile(file=filename, width=10)

 toplot=crablandings[crablandings$cfa0==c,]
 plot=barplot(toplot$mt, main= paste(toplot$year[1],toplot$cfa0[1],"Landings", sep=" "),cex.main=1.8, xlab= "Week of Season",
...
pdf(file=filename)
        ...
        plot(spring$year, spring$perc, type="n", cex.main=1.8,
        ylab="Percent of Total", xlab="Year", ylim=c(0,100), lty=1, col="red", pch=19, bg="white")
...
write.table(days_fished,file="days_fished.csv", sep=",")


check<-days_fished %>%
  filter(years == '2020', cfa0 == 'cfa4X') %>%
  select
...
cpue = compute.catchrate( x=logs.fixed, var="catchrate", index=c("years","area" ) )
...
cpu = compute.catchrate( x=logs.fixed, var="catchrate", index=c("years","cfa0" ) )
...
#, main= "Annual Catch Rates"

plot(cpu$year, cpu$cpuekg, type="n", cex.main=1.8,
ylab="Catch Rate (kg/ trap)", xlab="Year", ylim=c(0,1.2*(max(cpu$cpuekg))), lty=1, col="red", pch=19)
...
#i=cpu.quarter$cfa0%in%c("cfa23","cfa24", "north")
cpu=cpu.quarter[i,]
cpu=cpu[is.finite(cpu$catchrate),]
...
smoothcatch=function(x){
     plot(recent$time,recent$cpuekg , type="n", cex.main=1.8, ylab="Catch Rate (kg/ trap)",
...
monthlycpue = compute.means( x=logs.fixed, var="lbspertrap", index=c("month", "cfa0")  )
monthlycpue$lbspertrap[!is.finite(monthlycpue$lbspertrap)]=0
monthlycpue$kg=NA
monthlycpue$kg=monthlycpue$lbspertrap/2.204626
#monthlycpue=monthlycpue[is.finite(monthlycpue$lbspertrap),]
...
plotmonthlycpue=function(x){
 win.graph(width = 14, height = 8)
 toplot=c23 #change for each cfa to create CPUE by month
 genplot=plot(toplot$lbspertrap, main= paste(iy,"Monthly","Catch Rate", sep=" "),
 cex.main=1.8, xlab= "Month", type="n",axes=F, ylab="Catch Rate (lbs/trap)",
 ylim=c(0,1.2*(max(monthlycpue$lbspertrap))), col="blue")
...
###########this install is not working#################
install.packages("devtools")
devtools::install_github("Beothuk/bio.utilities")
...
boats = compute.vessels( x=logs, var="vessels", index=c("years", "cfa0")  )
boats=boats[is.finite(boats$vessels) & boats$years>2004,]
...
fishmap=function(z){
  makemap(x,area=a, title="Reported Fishing Positions", addlabels=F)
  last= logs[iyear1,]
  last=as.EventData(last, projection= "LL")
  addPoints(data=last, col="yellow", pch=20, cex=0.8)
  current= logs[iyear,]
  current=as.EventData(current, projection= "LL")
  addPoints(data=current, col="red", pch=20, cex=0.8)
  coverup(area=a)
  legend("bottomright", paste(c(iy1, iy)), pch=20, col=c("yellow","red"), bty="o", bg="grey75", pt.cex=1.5)
}
...
springmap=function(z){
  makemap(x,area=a, title="Spring Fishing Positions", addlabels=F)
  last= logs[iyear1spring,]
  last=as.EventData(last, projection= "LL")
  addPoints(data=last, col="yellow", pch=20, cex=0.8)
  current= logs[iyearspring,]
  current=as.EventData(current, projection= "LL")
  addPoints(data=current, col="red", pch=20, cex=0.8)
  coverup(area=a)
  legend("bottomright", paste(c(iy1, iy)), pch=20, col=c("yellow","red"), bty="o", bg="grey75", pt.cex=1.5)
...
summermap=function(z){
    makemap(x,area=a, title="Summer Fishing Positions", addlabels=F)
    last= logs[iyear1sum,]
    last=as.EventData(last, projection= "LL")
    addPoints(data=last, col="yellow", pch=20, cex=0.8)
    current= logs[iyearsum,]
    current=as.EventData(current, projection= "LL")
    addPoints(data=current, col="red", pch=20, cex=0.8)
    coverup(area=a)
    legend("bottomright", paste(c(iy1, iy)), pch=20, col=c("yellow","red"), bty="o", bg="grey75", pt.cex=1.5)
...
b=dbGetQuery(con, ("SELECT * FROM SNCRABSETS_OBS"))
...
x$BOARD_DATE[i4x]=x$BOARD_DATE[i4x]-7776000 #subtracts 91 days from all dates ( 7776000seconds)
x$years=as.character(years(as.chron(x$BOARD_DATE)))        #determine year
x$BOARD_DATE[i4x]=x$BOARD_DATE[i4x]+7776000 #adds 90 days


#Choose most recent year and create directory
i=which(x$cfa=="CFA 23")
iy=max(as.numeric(as.character(x$year)))
#iy=max(as.numeric(as.character(x$year[i])))

zz=paste("C:/Rsaves/fishery", iy, sep="/")
dir.create(zz)
setwd(zz)
wd=zz
...
setsobs=dbGetQuery(con, ("SELECT * FROM SNCRABSETS_OBS, SNCRABDETAILS_OBS
...
write.table(soft.sum,file="softsummary.csv", sep=",")

#Save a pdf version of this table
source("C:/Scripts/functions/tab.4.tex.r")
pdf(file = "softsummary.pdf", width=6, height=3)
...
xyear$X=xyear$lon
xyear$Y=(xyear$lat)
xyear$EID=as.numeric(paste(1:nrow(xyear),  sep=""))
...
# NENS Soft Crab Map
        ...
        northsoft=function(){
        makemap(xyear, area="nens", addlabels=F,title=paste(xyear$year[1]))
        addPoints(data=xyear[xyear$color=="green4",], col= "black", bg="green4", pch=21)
        addPoints(data=xyear[xyear$color=="yellow",], col= "black", bg="yellow", pch=21)
        addPoints(data=xyear[xyear$color=="red",], col= "black", bg="red", pch=21)
...
# CFA 23 Soft Crab Map
 ...
 area23soft=function(){
        makemap(xyear, area="cfa23zoom", addlabels=F,title=paste(xyear$year[1], "Soft Crab", sep=" "))
        addPoints(data=xyear[xyear$color=="green4",], col= "black", bg="green4", pch=21)
        addPoints(data=xyear[xyear$color=="yellow",], col= "black", bg="yellow", pch=21)
        addPoints(data=xyear[xyear$color=="red",], col= "black", bg="red", pch=21)

        points(x=-58.25, y=44.4,  col="black", bg="green4", pch=21, cex=1.1) # add legend like key
        text("0-10% ", x= -58.2, y=44.4, font=2, cex=.85, pos=4, col="black")
        points(x=-58.25, y=44.3,  col="black", bg="yellow", pch=21, cex=1.1) # add legend like key
        text("10-20% ", x= -58.2, y=44.3, font=2, cex=.85, pos=4, col="black")
        points(x=-58.25, y=44.2,  col="black", bg="red", pch=21, cex=1.1) # add legend like key
        text("20+ %", x= -58.2, y=44.2, font=2, cex=.85, pos=4, col="black")
        }
...
area24soft=function(){
        makemap(xyear, area="cfa24zoom", addlabels=F,title=paste(xyear$year[1], "Soft Crab", sep=" "))
        addPoints(data=xyear[xyear$color=="green4",], col= "black", bg="green4", pch=21)
        addPoints(data=xyear[xyear$color=="yellow",], col= "black", bg="yellow", pch=21)
        addPoints(data=xyear[xyear$color=="red",], col= "black", bg="red", pch=21)

        points(x=-62.05, y=44.1,  col="black", bg="green4", pch=21, cex=1.1) # add legend like key
        text("0-10% Soft", x= -62, y=44.1, font=2, cex=.85, pos=4, col="black")
        points(x=-62.05, y=44,  col="black", bg="yellow", pch=21, cex=1.1) # add legend like key
        text("10-20% Soft", x= -62, y=44, font=2, cex=.85, pos=4, col="black")
        points(x=-62.05, y=43.9,  col="black", bg="red", pch=21, cex=1.1) # add legend like key
        text("20+ % Soft", x= -62, y=43.9, font=2, cex=.85, pos=4, col="black")
        }
...
lfobsquery=dbGetQuery(con, ("SELECT SNCRABDETAILS_OBS.TRIP_ID, SNCRABDETAILS_OBS.TRIP,
...
#4X fishing activity on 4X line doesn't get recoded properly by recode.areas
#force thes into 4X
	x$month=months(as.chron(x$BOARD_DATE))
	x$long=NA
	x$long=-(as.numeric(x$lon))
	i4x=which(x$month %in% c("Nov", "Dec", "Jan", "Feb") & x$long>63.3)
	x$cfa[i4x]="4X"
...
# i.e. Winter 2011 gets grouped under 2010 "Season"
	i4x=which(x$cfa %in% "4X")
	x$BOARD_DATE[i4x]=x$BOARD_DATE[i4x]-7776000 #subtracts 91 days from all dates in x ( 7776000seconds)
	x$year=as.character(years(as.chron(x$BOARD_DATE)))        #determine year
	x$BOARD_DATE[i4x]=x$BOARD_DATE[i4x]+7776000 #adds 90 days to 4X samples
...
meansizes = compute.sums( x=x, var="FISH_LENGTH", index=c("year", "cfa")  )
...
cwplot=function(){
        cf=unique(toplot$cfa)
        cols=c("black", "red", "cornflowerblue","green4")
        point=c(1,2,4,15)


        plot(toplot$year, toplot$cw, type="n", main= "Mean CW Observed", cex.main=1.2,
        ylab="Mean CW(mm)", xlab="Year", ylim=c(0.8*(min(toplot$cw, na.rm=T)),1.2*(max(toplot$cw, na.rm=T))), lty=1, col="red", pch=19)

        for (y in 1:length(cf)) {
         c=toplot[toplot$cfa==cf[1],]
         c=toplot[toplot$cfa==cf[y],]
...
baseplot=function(){
 plot(base$cw, base$mass, type="l", main= "Weight at Size", cex.main=1.2,
        ylab="Mass(g)", xlab="CW(mm)", ylim=c(min(base$mass, na.rm=T),1.2*(max(base$mass, na.rm=T))),
...
#Create a plot of mean mass by year for each area
       ...
       massplot=function(){
        plot(toplot$year, toplot$mass, type="n", main= "Calculated Mean Weight", cex.main=1.2,
        ylab="Mass(g)", xlab="Year", ylim=c(0.8*(min(toplot$mass, na.rm=T)),1.2*(max(toplot$mass, na.rm=T))), lty=1, col="red", pch=19)
...
a=all[all$yr==iy,]
filename=paste(iy, "_all_survey_stations", ".pdf", sep="")
...
TAC = c(3280, 3604, 4414, 2784, 3059, 3747, 0, 55, 80, 784, 630, 848)
table1a <-cbind(table1, TAC)
#names(table1a)[1]<-"Area"
#names(table1a)[2]<-"Year"
names(table1a)[5]<-"TAC"
#names(table1a)[4]<-"Catchrate"
#names(table1a)[5]<-"Landings"



table1a<-table1 %>%
  select(area, years, mt, catchrate) %>%
  #filter(area != "cfa4X") %>%
...
table1a%>%
  kbl(caption = "Landings.") %>%
  kable_classic()

write.csv(table1, "landing_data2.csv", row.names = FALSE)

write.table(north,file="North monthly cpue.csv", sep=",")
write.table(c23,file="c23 monthly cpue.csv", sep=",")
write.table(c24,file="c24 monthly cpue.csv", sep=",")
write.csv(wk, "wk.csv", row.names = FALSE)
write.csv(logs.fixed, "logcheck.csv", row.names = FALSE)
}
